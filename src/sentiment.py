...
def get_review_sentiment(review, tokenizer, model):
    inputs = tokenizer(str(review), return_tensors="pt", truncation=True, padding=True)
    with torch.no_grad():
        outputs = model(**inputs)
        pred = torch.argmax(outputs.logits, dim=1).item()
    return pred  # 0 = negative, 1 = positive (depends on model)

def predict_product_sentiments(input_file="data/processed.csv", output_file="output/flipkartsentiment.xlsx", model_name="roberta-base"):
    ...
    # Add sentiment column for each review
    df["sentiment_score"] = df["processed_review"].apply(lambda x: get_review_sentiment(x, tokenizer, model))
